...
import { useQuiz } from './QuizContext'; // Importar Contexto

const BancoDePerguntas = ({ onAdicionarPergunta, perguntas, onExcluir, onEditar }) => {
  const { t } = useQuiz(); // Usar o hook t()
  ...
  const lidarComExclusao = (id) => {
    // Texto traduzido no confirm
    if (window.confirm(`${t('msg_confirmar_exclusao')} ${id}?`)) {
      onExcluir(id); 
    }
  };
  ...
  return (
    <div className="bg-white dark:bg-gray-900 p-0 rounded-xl transition-colors duration-300">
      <header className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-semibold text-gray-800 dark:text-white">{t('banco_titulo')}</h2>
        <button 
        ...
        >
          <span className="text-xl mr-1">+</span> {t('btn_adicionar')}
        </button>
      </header>
      ...
      <div className="flex space-x-4 mb-6">
        ...
        >
          {categoriasUnicas.map(cat => (
            // Tenta traduzir a categoria, se não achar, exibe o texto original
            <option key={cat} value={cat}>
                {cat === 'Todas as Categorias' ? t('todas_categorias') : cat}
            </option>
          ))}
        </select>
      </div>
      ...
      <div className="overflow-x-auto bg-gray-100 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-50 dark:bg-gray-700">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-3/5">{t('tab_pergunta')}</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-1/5">{t('tab_categoria')}</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-1/5">{t('tab_acoes')}</th>
            </tr>
          </thead>
