...
export default function Categoria() {
  ...
  function sortearPerguntasPorCategoria(categoria) {
    const filtradas = perguntasDoBanco.filter((p) => p.categoria.toLowerCase() === categoria.toLowerCase());
  ...
  function handleSelecionar(categoria) {
    ...
    if (selecionadas.length === 0) {
      alert("Ainda não há perguntas cadastradas nesta categoria!");
      return;
    }
  ...
  function getCardClasses(categoriaKey) {
    ...
    return (
      (invisivel ? "opacity-0 " : "opacity-100 ") + baseCard + " " + (isSelected ? selectedBorder : normalBorder) + " " + slideClass
    );
  }

  if (loading) return <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-[#020617] text-gray-800 dark:text-white">Carregando categorias...</div>;

  return (
    // Fundo muda conforme o tema: bg-slate-50 (Claro) vs bg-[#020617] (Escuro)
    <div className="min-h-screen w-full bg-slate-50 dark:bg-[#020617] font-display text-gray-900 dark:text-white transition-colors duration-300">
      <div className="relative flex min-h-screen w-full flex-col items-center overflow-hidden p-4 sm:p-6 md:p-8">
        ...
        {/* Glow apenas no dark mode ou sutil no light */}
        ...
        <main className="flex flex-1 flex-col items-center justify-center w-full max-w-5xl py-12 sm:py-16 md:py-20">
          <div className={`flex flex-col items-center gap-10 w-full transform transition-all duration-300 ease-out ${containerAnim}`}>
            <div className="flex flex-col items-center gap-2">
              <h1 className="tracking-tight text-4xl sm:text-5xl font-bold leading-tight text-center">Escolha uma categoria</h1>
              <p className="text-gray-600 dark:text-gray-300 text-base font-normal leading-normal text-center max-w-md">
                Teste seus conhecimentos com perguntas atualizadas!
              </p>
            </div>
            ...
            <div className="grid w-full grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 p-4">
              <button type="button" onClick={() => handleSelecionar("Filantrópicas")} className={getCardClasses("Filantrópicas")}>
                ...
                <div className="flex flex-col gap-1 text-left">
                  <h2 className="text-lg font-bold">Filantrópicas</h2>
                  <p className="text-gray-500 dark:text-gray-300 text-sm">Atividades de caridade</p>
                </div>
              </button>
              ...
              <button type="button" onClick={() => handleSelecionar("Recreativas")} className={getCardClasses("Recreativas")}>
                ...
                <div className="flex flex-col gap-1 text-left">
                  <h2 className="text-lg font-bold">Recreativas</h2>
                  <p className="text-gray-500 dark:text-gray-300 text-sm">Atividades de lazer</p>
                </div>
              </button>
              ...
              <button type="button" onClick={() => handleSelecionar("Esportivas")} className={getCardClasses("Esportivas")}>
                ...
                <div className="flex flex-col gap-1 text-left">
                  <h2 className="text-lg font-bold">Esportivas</h2>
                  <p className="text-gray-500 dark:text-gray-300 text-sm">Atividades esportivas</p>
                </div>
              </button>
              ...
              <button type="button" onClick={() => handleSelecionar("Culturais")} className={getCardClasses("Culturais")}>
                ...
                <div className="flex flex-col gap-1 text-left">
                  <h2 className="text-lg font-bold">Culturais</h2>
                  <p className="text-gray-500 dark:text-gray-300 text-sm">Atividades culturais</p>
                </div>
              </button>
            </div>
          </div>
